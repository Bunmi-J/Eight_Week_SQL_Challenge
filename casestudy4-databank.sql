...
SELECT r.region_id, r.region_name, COUNT(DISTINCT cn.node_id) AS nodes_per_regions
FROM customer_nodes cn
JOIN regions r
ON cn.region_id = r.region_id
GROUP BY r.region_id, r.region_name
ORDER BY nodes_per_regions DESC;


-- 3 -How many customers are allocated to each region?
SELECT COUNT(DISTINCT cn.customer_id) AS customers_per_region, r.region_name
FROM customer_nodes cn
JOIN regions r
ON cn.region_id = r.region_id
...
SELECT ROUND(AVG(EXTRACT(DAY FROM AGE(end_date, start_date)))::NUMERIC, 0) AS avg_reallocation_days
FROM customer_nodes
...
WITH reallocation_days AS (
    SELECT region_id, 
           EXTRACT(DAY FROM AGE(end_date, start_date)) AS days_reallocated
    FROM customer_nodes
    WHERE end_date IS NOT NULL
)
SELECT r.region_name,
       PERCENTILE_CONT(0.50) WITHIN GROUP (ORDER BY rd.days_reallocated) AS median_days,
       PERCENTILE_CONT(0.80) WITHIN GROUP (ORDER BY rd.days_reallocated) AS percentile_80,
       PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY rd.days_reallocated) AS percentile_95
FROM reallocation_days rd
...
WITH monthly_transactions AS (
    SELECT customer_id, 
           ...
           COUNT(*) FILTER (WHERE txn_type IN ('purchase', 'withdrawal')) AS purchase_or_withdrawal_count
    FROM customer_transactions
    GROUP BY customer_id, txn_month
)
SELECT txn_month, COUNT(DISTINCT customer_id) AS qualifying_customers
...
WITH monthly_balances AS (
    SELECT customer_id, 
           ...
           SUM(CASE WHEN txn_type IN ('withdrawal', 'purchase') THEN txn_amount ELSE 0 END) AS total_spent
    FROM customer_transactions
    GROUP BY customer_id, txn_month
),
running_balances AS (
    SELECT customer_id, txn_month,
           SUM(total_deposits - total_spent) OVER (PARTITION BY customer_id ORDER BY txn_month) AS closing_balance
...
WITH closing_balances AS (
    SELECT customer_id, 
           ...
           OVER (PARTITION BY customer_id ORDER BY DATE_TRUNC('month', txn_date)) AS closing_balance
    FROM customer_transactions
),
pct_increase AS (
    SELECT customer_id, txn_month, closing_balance,
           LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY txn_month) AS prev_closing_balance,
           100 * (closing_balance - LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY txn_month)) 
           / NULLIF(LAG(closing_balance) OVER (PARTITION BY customer_id ORDER BY txn_month), 0) AS pct_increase
    FROM closing_balances
)
SELECT ROUND(100.0 * COUNT(DISTINCT customer_id) / (SELECT COUNT(DISTINCT customer_id) FROM customer_transactions), 2) AS pct_customers
FROM pct_increase
WHERE pct_increase > 5;


--C. Data Allocation Challenge
--Real Time Running customer balance
...
SELECT customer_id, DATE_TRUNC('month', txn_date) AS month_end,
       LAST_VALUE(running_balance) OVER (PARTITION BY customer_id ORDER BY txn_date RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS end_of_month_balance
FROM (
    SELECT customer_id, txn_date, txn_type, txn_amount,
           SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) 
           OVER (PARTITION BY customer_id ORDER BY txn_date) AS running_balance
...
GROUP BY customer_id, month_end, running_balance, txn_date;

--Average Balance in 30 days
SELECT customer_id, txn_date,
       AVG(running_balance) OVER (PARTITION BY customer_id ORDER BY txn_date ROWS BETWEEN 30 PRECEDING AND CURRENT ROW) AS avg_30_day_balance
FROM (
    SELECT customer_id, txn_date, txn_type, txn_amount,
           SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) 
           OVER (PARTITION BY customer_id ORDER BY txn_date) AS running_balance
...
-- Data Based on End-of-Month Balance
WITH monthly_balances AS (
    SELECT customer_id, DATE_TRUNC('month', txn_date) AS month_end,
           LAST_VALUE(running_balance) OVER (PARTITION BY customer_id ORDER BY txn_date RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS end_of_month_balance
    FROM (
...
SELECT month_end, SUM(end_of_month_balance * 0.5) AS total_data_required
FROM monthly_balances
GROUP BY month_end
ORDER BY month_end;

--OPTION 2- Data Based on 30-Day Average Balance
...
SELECT DATE_TRUNC('month', txn_date) AS month, SUM(avg_30_day_balance * 0.5) AS total_data_required
FROM avg_balances
GROUP BY month
ORDER BY month;

--OPTION 3 -DATA BASED ON REAL TIME BALANCE
SELECT DATE_TRUNC('month', txn_date) AS month, 
       SUM(running_balance * 0.5) AS total_data_required
FROM (
    SELECT customer_id, txn_date, txn_type, txn_amount,
           SUM(CASE WHEN txn_type = 'deposit' THEN txn_amount ELSE -txn_amount END) 
           OVER (PARTITION BY customer_id ORDER BY txn_date) AS running_balance
